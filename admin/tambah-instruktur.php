...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO instruktur(nama_instruktur, bidang,  foto) VALUES('$nama_instruktur','$bidang','$nama_foto')");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO instruktur(nama_instruktur, bidang, foto) VALUES('$nama_instruktur','$bidang','$nama_foto')");
    }
...
if (isset($_POST['edit'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            echo "Extension gambar tidak ditemukan";
            die;
        } else {
            unlink('upload/' . $rowEdit['foto']);
            move_uploaded_file($_FILES['foto']['tmp_name'], 'upload/' . $nama_foto);
            $update = mysqli_query($koneksi, "UPDATE instruktur SET nama_instruktur='$nama_instruktur', bidang='$bidang', foto ='$nama_foto' WHERE id = '$id'");
        }

        //coding ubah/update disini
    } else {
        // kalau user tidak ingin memasukkan gambar
        $update = mysqli_query($koneksi, "UPDATE instruktur SET nama_instruktur='$nama_instruktur', bidang='$bidang'  WHERE id = '$id'");
    }
                                        ...
                                        <form action="" method="POST" enctype="multipart/form-data">
                                            <div class="mb-3 row">
                                                ...
                                                <div class="col-sm-6">
                                                    ...
                                                    <input type="text" class="form-control" name="bidang" placeholder="Masukkan Bidang" required value="<?php echo isset($_GET['edit']) ? $rowEdit['bidang'] : '' ?>">
                                                </div>
                                                <!-- <div class="mb-3-row">
                                                    <div class="col-sm-12">
